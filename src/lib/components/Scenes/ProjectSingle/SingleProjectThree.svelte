<script lang="ts">
	...
	onMount(() => {
		...
		if (gameVideo) {
			let gameVideo = document.getElementById(
				'gameVideo'
			) as HTMLVideoElement
			gameVideo.play()
	...
	useFrame(() => {
		...
		if (!mouseDown && ($isMobile ? speed < 1 && speed > -1 : true))
			position += Math.sign(diff) * Math.pow(Math.abs(diff), 0.7) * 0.06
	...
	function infinitePictures() {
		...
		ref.children[0].children.forEach((picture, i) => {
			picture.position.y = -(
				((margin * i + 2 + calcPos + 42069 * wholeHeight) %
					wholeHeight) -
				wholeHeight / 2
			)
		})
	...
	function updatePlaneTexture() {
		if (textures) {
			speedLerp = lerp(speedLerp, targetSpeed, 0.6)
			ref.children[0].children.forEach((mesh: any) => {
				mesh.material.uniforms.uOffset.value =
					(targetSpeed - speedLerp) * 0.0008
			})
		}
	}
</script>
